...
### Incremental review update (BusOp-size + multithread liveness checkpoint)
- Added centralized transfer-size validation in `BusArbiter`: non-barrier ops now only accept 1/2/4-byte transfers, with invalid sizes deterministically rejected as no-side-effect/no-stall responses.
- Added focused kernel regressions that pin invalid-size rejection semantics for both RAM and MMIO paths (no mutation + deterministic zeroed read response for invalid-size MMIO read).
- Added a deterministic multithread stress regression over an extended SCU contention script to validate `run_scripted_pair_multithread` liveness/progress under repeated contention windows and to pin exact trace stability across repeated runs.
- Residual risk: MMIO lane mapping currently uses a temporary split policy (big-endian general MMIO with legacy little-endian SCU window compatibility), so future cleanup should converge this to a single lane model and refresh SCU fixtures together.
