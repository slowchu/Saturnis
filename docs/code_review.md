...
1. **Device model breadth is still narrow**
   - Only a small number of explicit MMIO registers have behavior; broader SMPC/SCU/VDP/SCSP maps remain unspecified.
...
1. SCU register set expansion (deterministic masks + status bits + tests).
