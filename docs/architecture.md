...
## SH-2 ISA coverage refresh (batch ISA rev2)

- Core register file now includes `GBR`, `VBR`, `MACH`, and `MACL` as architecturally visible SH-2 control/accumulator registers.
- PC-relative addressing follows: `pc_` points at current 16-bit opcode; `@(disp,PC)` base is `pc_ + 4`; longword PC-relative forms align base with `& ~3`; branch disp targets use `(pc_ + 4) + (signext(disp) * scale)`.
- Exception entry and return model stack-based flow through bus-visible operations: exception/TRAPA push `SR` and return `PC` to `@-R15`, vector from `VBR + vector*4`, and `RTE` restores `PC`/`SR` from stack.
- Newly supported addressing forms include `@(disp,PC)` (`MOV.W`/`MOV.L` + `MOVA`), pre-decrement stores (`MOV.B/W/L Rm,@-Rn`), and register-displacement `@(disp,Rm)` / `@(disp,Rn)` variants.
