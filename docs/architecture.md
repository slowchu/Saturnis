...
- Bus transfer-size validation is centralized in the arbiter for deterministic rejection semantics: non-barrier ops only model 1/2/4-byte transfers; invalid-size requests are rejected without state mutation.
- Multithread scripted-pair liveness is regression-checked with an extended deterministic contention script that stresses progress/response mailbox coordination under repeated contention windows.
