...
std::uint32_t DeviceHub::read(std::uint64_t, int, std::uint32_t addr, std::uint8_t size) {
  ...
  const std::uint32_t persisted_value = (it != mmio_regs_.end()) ? it->second : 0U;
  const std::uint32_t value = materialize_register_value(spec, persisted_value);
...
void DeviceHub::write(std::uint64_t t, int cpu, std::uint32_t addr, std::uint8_t size, std::uint32_t value) {
  ...
  const std::uint32_t next_value = (persisted_value & ~masked_write) | ((value << shift) & masked_write);
