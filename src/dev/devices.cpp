...
constexpr std::uint32_t kScuDma0ReadAddr = 0x05FE0020U;
...
[[nodiscard]] std::optional<MmioRegisterSpec> register_spec(std::uint32_t word_addr) {
  ...
  if (word_addr == kScuDma0ReadAddr) {
    return MmioRegisterSpec{0x0U, 0xFFFFFFFFU};
  }
  if (word_addr == kScuDma0SizeAddr) {
    return MmioRegisterSpec{0x0U, 0x000FFFFFU};
  }
  if (word_addr == kScuDma0CtrlAddr) {
    return MmioRegisterSpec{0x0U, 0x00000017U};
  }
