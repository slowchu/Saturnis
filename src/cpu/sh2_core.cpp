...
bool SH2Core::decode_mov_store_disp(std::uint16_t instr, std::uint8_t &n, std::uint8_t &m, std::uint8_t &size,
                                    std::uint32_t &disp_bytes) const {
  if ((instr & 0xF000U) == 0x1000U) {
    n = static_cast<std::uint8_t>((instr >> 8U) & 0x0FU);
    m = static_cast<std::uint8_t>((instr >> 4U) & 0x0FU);
    const std::uint8_t disp = static_cast<std::uint8_t>(instr & 0x0FU);
    size = 4U;
    disp_bytes = static_cast<std::uint32_t>(disp) * 4U;
    return true;
  }

  if ((instr & 0xF00FU) == 0x8004U || (instr & 0xF00FU) == 0x8005U) {
    n = static_cast<std::uint8_t>((instr >> 8U) & 0x0FU);
    m = 0U;
    const std::uint8_t disp = static_cast<std::uint8_t>((instr >> 4U) & 0x0FU);
    size = static_cast<std::uint8_t>((instr & 0x000FU) == 0x4U ? 1U : 2U);
    disp_bytes = static_cast<std::uint32_t>(disp) * static_cast<std::uint32_t>(size);
    return true;
  }
...
bool SH2Core::decode_mov_load_disp(std::uint16_t instr, std::uint8_t &n, std::uint8_t &m, std::uint8_t &size,
                                   std::uint32_t &disp_bytes) const {
  ...
  if ((instr & 0xF00FU) == 0x8000U || (instr & 0xF00FU) == 0x8001U) {
    n = 0U;
    m = static_cast<std::uint8_t>((instr >> 8U) & 0x0FU);
    const std::uint8_t disp = static_cast<std::uint8_t>((instr >> 4U) & 0x0FU);
    size = static_cast<std::uint8_t>((instr & 0x000FU) == 0x0U ? 1U : 2U);
    disp_bytes = static_cast<std::uint32_t>(disp) * static_cast<std::uint32_t>(size);
    return true;
  }
...
std::optional<bus::BusOp> SH2Core::execute_instruction(std::uint16_t instr, core::TraceLog &trace, bool from_bus_commit) {
  ...
  if (decode_mov_store_disp(instr, n, m, size, disp_bytes)) {
    const bool r0_source = ((instr & 0xF00FU) == 0x8004U || (instr & 0xF00FU) == 0x8005U);
    const std::uint32_t value = r0_source ? r_[0] : r_[m];
    const std::uint32_t vaddr = r_[n] + disp_bytes;
    const std::uint32_t phys = mem::to_phys(vaddr);
    const bool cacheable = cacheable_data(vaddr, phys);
  ...
  if (decode_mov_load(instr, n, m, size) || decode_mov_load_postinc(instr, n, m, size) ||
      decode_mov_load_disp(instr, n, m, size, disp_bytes)) {
    const bool post_inc = decode_mov_load_postinc(instr, n, m, size);
    const bool disp_mode = decode_mov_load_disp(instr, n, m, size, disp_bytes);
    const std::uint32_t vaddr = disp_mode ? (r_[m] + disp_bytes) : r_[m];
    const std::uint32_t phys = mem::to_phys(vaddr);
    const bool cacheable = cacheable_data(vaddr, phys);
    if (cacheable) {
