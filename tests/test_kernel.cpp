...
void test_sh2_data_displacement_forms() {
  saturnis::core::TraceLog trace;
  saturnis::mem::CommittedMemory mem;
  saturnis::dev::DeviceHub dev;
  saturnis::bus::BusArbiter arbiter(mem, dev, trace);
  ...
  // mov.b @(2,r4),r0
  // nop
  const std::uint16_t prog[] = {0xE140U, 0xE22AU, 0x1121U, 0x5311U, 0xE450U, 0xE007U, 0x8424U, 0x8420U, 0x0009U};
  for (std::size_t i = 0; i < (sizeof(prog) / sizeof(prog[0])); ++i) {
    mem.write(static_cast<std::uint32_t>(i * 2U), 2U, prog[i]);
  }
  ...
  check(core.reg(3) == 0x2AU, "mov.l displacement load should read stored dword");
  check((core.reg(0) & 0xFFU) == 0x07U, "mov.b displacement load should read stored byte");
  check(mem.read(0x44U, 4U) == 0x2AU, "mov.l displacement store should commit globally");
  check((mem.read(0x52U, 1U) & 0xFFU) == 0x07U, "mov.b displacement store should commit globally");
}
...
int main() {
  ...
  test_sh2_data_displacement_forms();
